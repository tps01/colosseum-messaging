...
First-party Colosseum plugin providing `col.messaging.*` (HTTP, Redis, ZMQ, MQTT).
...
This installs the complete plugin (HTTP via stdlib, Redis, ZMQ, and MQTT). It requires
...
libzmq is LGPL unmodified), and `paho-mqtt` under **EDL-1.0** (BSD-3 equivalent; dual-licensed
EPL-2.0 OR EDL-1.0).
